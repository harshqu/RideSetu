...
export default function CarRentalCityPage({ params }: { params: { city: string } }) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      <div className="bg-navy-950 text-white p-8 sm:p-12 rounded-3xl space-y-4 text-center max-w-4xl mx-auto">
        <span className="px-3 py-1 rounded-full bg-brand-orange text-white text-xs font-bold uppercase">
          Verified Self-Drive Car Rentals
        </span>
        <h1 className="text-3xl sm:text-4xl font-black font-heading tracking-tight">
          Self-Drive Car & Thar 4x4 Rental in {city}
        </h1>
        <p className="text-slate-300 text-sm max-w-2xl mx-auto leading-relaxed">
          Compare self-drive cars, Mahindra Thar 4x4, Swift, and Creta from verified operators in {city} with airport and railway delivery.
        </p>

        <div className="pt-4 max-w-xl mx-auto">
          <Link
            href={`/vehicles?destination=${citySlug}&category=CAR`}
      ...
      <div className="max-w-5xl mx-auto">
        <SearchWidget initialDestination={citySlug} initialCategory="CAR" />
      </div>
    </div>
  );
}

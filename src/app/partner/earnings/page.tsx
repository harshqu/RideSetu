    ...
    <div className="max-w-7xl mx-auto space-y-6 font-sans">
      <div className="border-b border-slate-200 pb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-black font-heading text-slate-900">Earnings Ledger & Settlement Breakdown</h1>
        </div>
      ...
      <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-start gap-3 text-xs text-emerald-900">
        ...
        <div>
          <strong className="font-extrabold text-emerald-950">Security Deposit Isolation Policy:</strong>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-2 shadow-sm">
              ...
              <div className="text-2xl font-black text-slate-900">{formatINR(grossRevenue)}</div>
              <div className="text-[11px] text-slate-500 font-medium">Total base rental fees</div>
            </div>

            <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-2 shadow-sm">
              <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Platform Commission (15%)</span>
              <div className="text-2xl font-black text-rose-700">-{formatINR(platformCommission)}</div>
              <div className="text-[11px] text-slate-500 font-medium">RideSetu marketplace fee</div>
            </div>

            <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-2 shadow-sm">
              <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">GST Tax (18%)</span>
              <div className="text-2xl font-black text-amber-700">{formatINR(gstTax)}</div>
              <div className="text-[11px] text-slate-500 font-medium">Collected on platform fee</div>
            </div>

            <div className="bg-white p-6 rounded-3xl border border-slate-200 space-y-2 shadow-sm">
              <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Net Vendor Payout</span>
              <div className="text-2xl font-black text-emerald-700">{formatINR(netPayout)}</div>
              <div className="text-[11px] text-slate-500 font-medium">Eligible for bank settlement</div>
            </div>
          </div>
        </>
      )}
